...
GLuint ShaderLoader::CreateShader(GLenum shaderType, const char* shaderName)
{
	...
	std::string fileText = ReadShaderFile(shaderName);
	GLuint shaderID = glCreateShader(shaderType);

	const char *shaderPointer = fileText.c_str();
	const int shaderLength = fileText.length();

	glShaderSource(shaderID, 1, &shaderPointer, &shaderLength);
